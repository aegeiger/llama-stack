...
"""
Merge Stainless configuration into OpenAPI spec to create openapi.generator.yml

This script takes:
1. client-sdks/stainless/openapi.yml - Base OpenAPI specification
2. client-sdks/stainless/openapi.stainless.yml - Stainless resource configuration
3. (Optional) patch file - Additional modifications to apply

And produces:
- client-sdks/openapi/openapi.generator.yml - OpenAPI spec enriched with x-operation-name and x-nesting-path vendor extensions
        ...
        try:
            ...
            if action == 'set':
                ...
                if isinstance(parent, dict):
                    ...
                    print(f"  ✓ Set {path} = {value if not isinstance(value, dict) else '{...}'}")
                else:
                ...
                if isinstance(current_value, dict) and isinstance(value, dict):
                    ...
                    print(f"  ✗ Cannot merge into {path}: not a dict")
...
def main():
    ...
    print("Extracting resource->endpoint mappings...")
    endpoint_map, collision_set = extract_resources(stainless_config)
    print(f"Found {len(endpoint_map)} endpoint mappings")
    print(f"Found {len(collision_set)} resource name collisions: {sorted(collision_set)}")

    # Enrich the OpenAPI spec
    print("Enriching OpenAPI spec with vendor extensions...")
