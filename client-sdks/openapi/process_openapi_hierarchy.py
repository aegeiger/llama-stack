...
def get_children_from_hierarchy(tag: str, hierarchy: dict) -> list[str]:
    """Get direct children of a tag from the hierarchy.

    Args:
        tag: Tag to find children for
        hierarchy: Full API hierarchy dictionary
...
def process_openapi(input_file: str, output_file: str, hierarchy_file: str) -> None:
    ...
    # Add child information to tags with endpoints
    print("\nAdding child tag information to operations...")
    for path, path_item in spec.get("paths", {}).items():
        for method in ["get", "post", "put", "delete", "patch", "options", "head", "trace"]:
            if method in path_item:
                operation = path_item[method]
                if "tags" in operation and operation["tags"]:
                    tag = operation["tags"][0]  # We already reduced to single tag
                    children = get_children_from_hierarchy(tag, api_hierarchy)
                    if children:
                        operation["x-child-tags"] = children
                        print(f"  {method.upper():6} {path:50} tag: {tag} -> children: {children}")
    ...
    if tags_without_endpoints:
        ...
        for tag in sorted(tags_without_endpoints):
            ...
            # Add children information if this tag has children
            if children:
                operation_spec["x-child-tags"] = children

            spec["paths"][dummy_path] = {"get": operation_spec}
            print(f"  Created: GET {dummy_path} for tag [{tag}] (children: {children})")
